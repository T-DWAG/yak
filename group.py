...
OUTPUT_DIR = "similar_photos"  # 输出目录
...
def find_similar_photos():
    ...
    # 准备CSV数据
    csv_data = []
    csv_headers = ['组别', '序号', '原始文件名', '新文件名', '原始ZIP路径', '来源ZIP文件', 'ZIP内相对路径', '目标路径']
    ...
    # 生成CSV文件
    csv_path = os.path.join(OUTPUT_DIR, "图片分组记录.csv")
    try:
    ...
    logger.info(f"完成！共找到 {group_count} 组相似照片")
    
    # 输出统计信息
    total_images = sum(len(g) for g in groups.values() if len(g) > 1)
    logger.info(f"总共处理了 {total_images} 张相似图片")
